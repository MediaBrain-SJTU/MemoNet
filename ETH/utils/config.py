...
class Config:
    ...
    def __init__(self, cfg_id, cfg_info, tmp=False, create_dirs=False):
        self.id = cfg_id
        ...
        # results dirs
        cfg_root_dir = '/tmp/memonet' if tmp else self.results_root_dir
        self.cfg_root_dir = os.path.expanduser(cfg_root_dir)

        self.cfg_dir = '%s/%s_%s' % (self.cfg_root_dir, cfg_id, cfg_info)
