import argparse
from trainer import test_selector as trainer_ae
...
def main(config):
    ...
    for i in range(100):
        prepare_seed(0)
        config.model_ae = './training/training_selector/your_file_name_here/model_ae_epoch_'+str(i)+'_2021-11-12'
        print(config.model_ae)
        t = trainer_ae.Trainer(config)
        # print('[M] start training selector[warm.up] for SDD dataset.')
        t.fit()
